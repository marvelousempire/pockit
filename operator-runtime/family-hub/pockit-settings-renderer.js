...
(function (global) {
  ...
  async function applyPockitSettings(delta) {
    ...
    if (delta.catalog_entitlements && global.PockitCatalogEntitlements) {
      try {
        const saved = await global.PockitCatalogEntitlements.saveEntitlements(delta.catalog_entitlements);
        results.local.push("catalog-entitlements");
        if (saved.warning) results.errors.push(saved.warning);
      } catch (e) {
        results.errors.push(e.message || String(e));
      }
    }
  ...
  const EXTENDED_TABS = [
    { id: "catalog", label: "Catalog", icon: "AppstoreAddOutlined", registry_domain: "pockit" },
  ...
  function renderExtendedTabPanel(tabId) {
    if (tabId === "catalog") {
      const ent = global.PockitCatalogEntitlements?.loadFromStorage?.() || { mode: "fleet" };
      return global.PockitCatalogSettings?.renderCatalogTabPanel(ent)
        || `<div class="pockit-settings-pane"><p class="cs-hint">Catalog settings loading…</p></div>`;
    }
  ...
  function bindExtendedTabActions(content, tabId) {
    if (tabId === "catalog") {
      global.PockitCatalogSettings?.hydrateCatalogTab?.(content);
      return;
    }
    if (tabId === "voice") {
  ...
  async function collectExtendedSettings(root) {
    ...
    const catalogPanel = root.querySelector('[data-settings-tab="catalog"]');
    if (catalogPanel && global.PockitCatalogSettings) {
      let base = {};
      try {
        base = JSON.parse(root.dataset.catalogEntJson || "{}");
      } catch {
        base = {};
      }
      delta.catalog_entitlements = global.PockitCatalogSettings.collectEntitlementsFromRoot(root, base);
    }
