...
(function () {
  ...
  function migrateLegacy(stored) {
    ...
    const skin = localStorage.getItem("nephew-pockit-skin");
    if (skin) {
      patch.appearance = { ...(patch.appearance || {}), skin };
    }
    return patch;
  }
  ...
  function save(next) {
    ...
    config.schema_version = 1;
    if (config.appearance?.skin && window.PockitAppearanceRegistry?.normalizeSkinId) {
      config.appearance.skin = window.PockitAppearanceRegistry.normalizeSkinId(config.appearance.skin);
    }
    if (config.theme?.mode && window.PockitAppearanceRegistry?.normalizeColorMode) {
      config.theme.mode = window.PockitAppearanceRegistry.normalizeColorMode(config.theme.mode);
    }
  ...
  function syncLegacyKeys(cfg) {
    try {
      ...
      if (cfg.appearance?.skin) localStorage.setItem("nephew-pockit-skin", cfg.appearance.skin);
    } catch {
      /* ignore quota */
    }
  }
  ...
  function apply(next) {
    ...
    window.PockitAppearance?.applyFromConfig?.(cfg);
    startIdleWatcher();
    window.__pockitSyncChromeChip?.();
    return cfg;
  }
  ...
  function renderSettingsPage() {
    ...
    return `<article class="cassette-settings-page pockit-config-page"><header class="cs-hero">
      ...
      <div class="cs-sections">
        ...
        <section class="cs-section"><h2>🎨 Appearance</h2>
        ${window.PockitAppearance?.renderAppearancePanel?.(cfg) || ""}
        </section>
  ...
  function bindSettingsPage(root, hooks) {
    ...
    const appearancePane = root.querySelector('[data-settings-tab="appearance"]');
    if (appearancePane) window.PockitAppearance?.bindAppearancePanel?.(appearancePane);
  ...
  function renderSettingsModalTabPanel(tabId) {
    ...
    if (tabId === "layout") {
      return `<div class="pockit-settings-pane">
        ...
        <h3 class="pockit-settings-pane__title">Door defaults</h3>
        <div class="cs-card">
          ${renderSelect("filter.default_scope", "Default door filter", cfg.filter.default_scope, [
            { value: "all", label: "All doors" },
            { value: "local", label: "Local only" },
    ...
    if (tabId === "appearance") {
      return window.PockitAppearance?.renderAppearancePanel?.(cfg)
        || `<div class="pockit-settings-pane"><p class="cs-hint">Appearance settings loading…</p></div>`;
    }
    if (tabId === "focus") {
      return `<div class="pockit-settings-pane">
  ...
  function bindSettingsTabActions(content, tabId) {
    ...
    if (tabId === "appearance") {
      window.PockitAppearance?.bindAppearancePanel?.(content);
      return;
    }
    if (tabId === "devices") {
