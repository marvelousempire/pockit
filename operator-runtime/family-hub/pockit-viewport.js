...
(function () {
  ...
  const SUITE_EMBED_MIN_TIER = "laptop";

  function isSuiteParentEmbed() {
    try {
      if (new URLSearchParams(location.search).get("suite_embed") === "1") return true;
    } catch (_) {
      /* invalid URL */
    }
    return false;
  ...
  function tierAtLeast(tier, floor) {
    const order = REG()?.TIERS?.map((t) => t.id) || [];
    const ti = order.indexOf(tier);
    const fi = order.indexOf(floor);
    if (ti < 0 || fi < 0) return tier;
    return ti < fi ? floor : tier;
  }
  ...
  function resolveCurrentTier() {
    ...
    let tier = reg.resolveTier(width, height, { coarsePointer: isCoarsePointer() });
    if (isSuiteParentEmbed()) {
      tier = tierAtLeast(tier, SUITE_EMBED_MIN_TIER);
    }
    return tier;
  }
