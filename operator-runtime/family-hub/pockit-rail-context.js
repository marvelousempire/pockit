(function (global) {
  ...
  async function resolveCassetteCommands(catalogEntry) {
    const fallback = {
      voice: [
        "voice-premium", "ensure-voice", "deploy-voice-premium-dgx",
        "ensure-spark-tts-weights-m5", "install-spark-tts-m5", "spark-tts-eval", "voice-cassette",
      ],
      "voice-cassette": [
        "voice-premium", "ensure-voice", "deploy-voice-premium-dgx",
        "ensure-spark-tts-weights-m5", "install-spark-tts-m5", "spark-tts-eval", "voice-cassette",
  ...
  async function buildVoiceRightRailSections(health = {}, cassetteEnt = null) {
    ...
    const stack =
      h.voiceStack ||
      global.ParakeetVoicePad?.getState?.()?.voiceStack ||
      global.ParakeetVoicePad?.getState?.()?.health?.voiceStack ||
      {};
    const sw = stack.emotion_swarmer || {};
    const dp = stack.double_pass || {};
    const tt = stack.turn_taking || {};
    const inference = stack.inference_backend || h.inferenceBackend || "auto";
    const higgs = stack.higgs_status || "scaffold";
    const swOk = sw.enabled !== false && sw.ok !== false;
    const dpOk = dp.enabled !== false;
    const bargeOk = tt.barge_in_enabled !== false;
    const brainOk = inference === "mac-ollama" || inference === "dgx-ollama";
    const higgsLive = higgs === "live";

    sections.push({
      section: "COMP processors",
      items: [
        healthTileItem(
          "voice-h-swarmer",
          "Emotion swarmer",
          "BulbOutlined",
          swOk ? "4-agent parallel merge live" : probing ? "Checking…" : "Disabled or warming",
          swOk,
          probing && !Object.keys(stack).length,
        ),
        healthTileItem(
          "voice-h-doublepass",
          "Double-pass",
          "AuditOutlined",
          dpOk
            ? (dp.skip_on_fast ? "Audit skips on fast lane" : "Draft + DGX audit")
            : "Off",
          dpOk,
          probing,
        ),
        healthTileItem(
          ...
          "Barge-in",
          "AudioOutlined",
          bargeOk ? "Interrupt while Nephew speaks" : "Off",
          bargeOk,
          probing,
        ),
        healthTileItem(
          ...
          `VAD ${tt.silence_ms ?? 600}ms silence`,
          true,
          probing,
        ),
        healthTileItem(
          ...
          "Inference",
          "ClusterOutlined",
          inference === "mac-ollama"
            ? "Mac Ollama fast lane"
            : inference === "dgx-ollama"
              ? "DGX Ollama (WireGuard)"
              : String(inference),
          brainOk,
          probing,
        ),
        healthTileItem(
          ...
          "Higgs Audio v3",
          "ThunderboltOutlined",
          higgsLive ? "DGX scaffold live" : "Scaffold — Holler speaks on M5",
          higgsLive,
          probing,
        ),
      ],
  ...
  async function buildKnowledgeRightRailSections(cassetteEnt = null) {
    ...
    sections.push({
      ...
      items: [
        {
          id: "know-panel-rag-console",
          title: "RAG Console",
          icon: "ControlOutlined",
          subtitle: "Pipeline · ingest · chunk · rerank",
          type: "knowledge-panel",
          _knowledgePanel: "rag-console",
        },
        {
          id: "know-h-rerank",
          title: "Reranker",
          icon: "SortAscendingOutlined",
          subtitle: infra.reranker_ok !== false ? "bge-reranker-v2-m3 :9201" : "Check :9201",
          type: "knowledge-health",
          _knowledgePanel: "rag-console",
        },
      ],
    ...
    const deployIds = new Set([
      "index-corpus",
      "index-corpus-full",
      "memory-fabric-reindex",
      "corpus-status",
      "vault-ingest-docs",
      "vault-sync-understandings",
  ...
  function buildVoiceLeftRailSections() {
    ...
    const ragItems = [
      { id: "voice-ctl-rag-hybrid", title: "Smart RAG", icon: "ClusterOutlined", subtitle: "Prefetch corpus · fast stream", type: "voice-rag", _voiceRag: "hybrid" },
