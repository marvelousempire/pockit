...
const HubSurface = (() => {
  ...
  function notifyVersionChrome() {
    ...
    document.querySelectorAll(".pockit-mobile-version-pill").forEach((btn) => {
      ...
      btn.title = showUpdate
        ? `${surfaceDisplayName(surface)} v${ver} — v${latest} available (click for changelog)`
  ...
  function syncUpdateBadge() {
    ...
    document.querySelectorAll("#pockit-version-invariant").forEach((btn) => {
      ...
      btn.title = show
        ? `${surfaceDisplayName(surface)} v${runningVersion || surface?.version || "—"} — v${latest} available (click to update)`
  ...
  function bindChangelogLinks(root = document) {
    root.querySelectorAll("[data-action=pad-changelog]").forEach((el) => {
      if (el.dataset.padChangelogBound) return;
      el.dataset.padChangelogBound = "1";
      el.addEventListener("click", (e) => {
        e.preventDefault();
        if (
          el.getAttribute("data-action") === "pockit-apply-update" ||
          (el.classList.contains("has-update") && updateAvailableVersion)
        ) {
          applyPockitUpdate(updateAvailableVersion);
          return;
        }
