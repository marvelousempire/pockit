...
async function loadPockitCatalog() {
  const sources = [
    "/api/v1/framework/pockit-catalog",
    "/api/v1/framework/launchpad-catalog",
    "/pockit-catalog.json",
    "/launchpad-catalog.json",
  ];
  let catalog = null;
...
function setEncompassIframeRoute(path, cassetteOrId) {
  ...
  const door = speakersDoorForCard(cassette) || manifest?.speakers_door || "http://search-my-engine.localhost/";
...
async function ensureEncompassManifest(cassetteId) {
  ...
  try {
    ...
    if (r.ok) {
      ENCOMPASS_MANIFESTS[cassetteId] = await r.json();
...
function scrubParentEmbedQueryParams(params) {
  let dirty = false;
  for (const key of ["cassette", "c", "player", "pockit_hud"]) {
...
function openMacAppCenter(appId) {
  ...
  if (app.door) {
    openMacAppDoor(appId, app.door);
    return;
  }
  setCassette(`mac-app-${appId}`);
}
...
function setEncompassPanel(panelId, cassette) {
  ...
  if (isKnowledgeHubCard(c)) {
    ...
    if (panelDef?.pad_module === "knowledge-hud" || ["chat", "inventory", "probe"].includes(ENCOMPASS_PANEL)) {
...
function pockitPlayerOptions() {
  ...
  for (const p of POCKIT_CATALOG.players.filter(catalogConsoleBrowsable)) {
...
function isNativeCtDeckEmbed(cassette) {
  ...
  if (isVoiceHubCard(cassette) || isKnowledgeHubCard(cassette) || isPromptLibraryHubCard(cassette) || isShipIntegrityHubCard(cassette) || isConfigurationsHubCard(cassette) || isAccessoryDeskHubCard(cassette) || isQuickDeskHubCard(cassette) || cassette.type === "voice" || cassette.type === "knowledge" || cassette.type === "prompt-library" || cassette.type === "ship-integrity" || cassette.type === "configurations" || cassette.type === "accessory-desk" || cassette.type === "quick-desk") {
    return false;
  }
...
function usesNativeCenterCanvas(cassette) {
  if (!cassette) return false;
  return isVoiceHubCard(cassette) || isKnowledgeHubCard(cassette) || isPromptLibraryHubCard(cassette) || isShipIntegrityHubCard(cassette) || isConfigurationsHubCard(cassette) || isAccessoryDeskHubCard(cassette) || isQuickDeskHubCard(cassette) || isOdysseusHubCard(cassette)
    || isEncompassNativePad(cassette)
    || cassette.type === "voice" || cassette.type === "knowledge" || cassette.type === "prompt-library" || cassette.type === "ship-integrity" || cassette.type === "configurations" || cassette.type === "accessory-desk" || cassette.type === "quick-desk" || cassette.type === "odysseus";
}
...
function setCassette(id, { pushHistory = true, force = false } = {}) {
  ...
  const routeId = resolved ? canonicalHashCassetteId(id) : "overview";
...
function initTheme() {
  ...
  applyTheme(saved);
  // Re-apply when the OS preference changes (only matters in "auto")
...
function onConsoleNavSelected(value) {
  ...
  if (value === "voice") {
    ...
    if (value === "nephew-deck" && CONSOLE_SELF_EMBED_IDS.has(value) && consoleProjectionHref(catalogPlayerById(value))) {
...
function consoleModalPlayerCards() {
  if (!POCKIT_CATALOG?.players) return [];
  return POCKIT_CATALOG.players.filter(catalogConsoleBrowsable).map((p) => {
    const count = (p.hosted_cassettes || []).length;
    return {
      value: p.id,
      title: p.name || p.id,
...
function renderOverview() {
  ...
  const sectionsBlock = `<div class="pockit-sections pockit-sections--${isTape() ? "tape-center" : "hub"}">${sectionsHtml}</div>`;
  return `
    <div class="pockit">
      ${heroBlock}
      ${pockitFilterBar()}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await Promise.all([loadCards(), loadFrameworkCatalog(), loadFrameworkLibrary(), loadPockitCatalog()]);
  mergeCatalogOperatorDoorsIntoCards();
  await ensureEncompassManifest("web-odysseus");
